...
https://www.math3d.org/1LQtJhbncZ
...
Gradient funkcije $f(x)$ v točki na izolipsi $C$ je vedno pravokoten na krivuljo izolipse $C$.

*Za nadaljevanje si moramo predstavljati naslednjo sliko. Naj bo $f$ funkcija, ki jo gledamo po neki krivulji $r(t)$. Naj velja da na neki točki velja $f(r(t_{0}))' = 0$. To lahko velja le če se ta krivulja, v tisti točki, premika v smeri neke izolipse, saj vemo da če hodimo po izolipsi se vrednost funkcije ne spreminja - to pomeni da mora biti smer krivulje $r(t)$ v točki kjer je $f(r(t_{0}))' = 0$ enaka neki smeri izolipse oz. da sta tangentni, ker pa vemo da je gradient pravokoten na izolipso, bo v tej točki pravokoten tudi na našo krivuljo v točki $f(r(t_{0}))'=0$*

*Mogoče je še bolj logično da če velja $f(r(t))'=0$ to pomeni da se krivulja "izravna" oz. dobimo nek ekstrem ali sedlo - važno je da je tanegnta v tej točki vodoravna. Če je tangenta vodoravna na to krivuljo, lahko poiščemo neko izolipso ki gre čez to točko, katere tangenta je tudi vodoravna, ker smer tangent izolipse določajo kako mora iti krivulja da se ji vrednost ne spremnija mora tudi naša krivulja $r(t)$ v točki, kjer je odvod enak 0, iti v isti smeri kot tangenta izolipse, torej se bosta prekrivali, torej bo gradient funkcije pravokoten na obe hkrati.*

> Naj bosta $f : \mathbb{R} \rightarrow \mathbb{R}^{n}$ diferenciabilna fun. in $r(t)$ polj. zvezna krivulja. Velja:
> $$\nabla f \perp r(t) \Leftrightarrow f(r(t))' = 0$$
> >[!|dokaz]+ Dokaz:
> > Vemo da velja
> > $$f(r(t))' = \nabla f \cdot (r_{1}(t)', r_{2}(t)')$$
> > 
> > kjer je $(r_{1}(t)', r_{2}(t)')$ tangenta na krivuljo. Torej če velja da je $f(r(t))' = 0$ potem sta tangenta in krivulja pravokotni.
> > 
> > To lahko še dodatno argumentiramo kot da vidimo da je odvod $f(r(t))$ enak dejanskemu gradientu, ki je pomanjšan glede na to kako stran kaže od tangente, torej če bi bila tangenta in gradient vzporedna potem bi bil odvod enak gradientu, bolj kot je tangenta obrnjena stran, manjši postane odvod, dokler ni tangenta pravokotna v katerm primeru je odvod enak $0$ saj se krivuljo premika pravokotno na naraščanja funkcije $f$.
...
$f(r(t))$ je sedaj območja krivulje $r(t)$ evaluirano na $f$, kjer je $r(t)$ enaka krivulja ji jo dobimo iz $g(r(t)) = C$. To pomeni da ko ima $f(r(t))$ v neki točki $r(t_{0})$ enak odvod kot neka izolipsa, kar je $0$  oz. $f(r(t_{0}))' = 0$, potem vemo da je v tisti točki gradient $f$ pravokoten na krivuljo oz. vektor $r'(t) = (\dot r_{1}(t), \dot r_{2}(t))$.


$$f(r(t))' = 0\Leftrightarrow \nabla f \perp r'(t)$$


Ker iščemo kandidate za ekstreme iščemo stacionarne točke $f(r(t))$ kar je ekvivalentno temu da je gradient $f$ pravokoten na odvod $r(t)$.

Če sta gradienta $g$ in $f$ oba pravokotna na $r$ v isti točki potem sta linearno odvisna
