...
**Enostaven graf** je graf brez zank, vzporednih, in usmerjenih povezav. *Kjer so zanke povezave same s seboj.*
...
Matrika je v neusmerjenem grafu vedno simetrična, v enostavnem pa so na diagonali ima same ničle, v enostavnem je vsota vrednosti vrstic oz. stolpcev za vozlišče $v_{i}$ je enaka stopnji $v_{i}$.

Če je graf utežen matrika sosednosti vsebuje vrednost uteži posamezne povezave.



**Incidenčna matrika** je matrika ki povezuje vozlišča in povezave grafa. 

Za graf $G = (V,E)$ z $n$ vozlišči in $m$ povezazavami je velikosti $n \times m$ kjer je $B_{ij} = 1$, če je vozlišče $v_{i}$ element povezave $e_{j}$ in $B_{ij} = 0$, če ni.
...
**Ekscentričnost** vozlišča $\text{ecc}(u) = \underset{v\, \in\, V}{\max}(d(u,v))$ oz. razdalja do najbolj oddaljenega vozlišča.
...
**Metrični prostor** je definiran kot par množice elementov $X$ in preslikavo $d : X \times X \rightarrow \mathbb{R}$ ki ji rečemo **metrika**.
...
> > 
> > Če obstaja cikel mora potekati izmenično iz $V_{1}$ v $V_{2}$. Da cikel obstaja se mora začeti in končati v isti množici, kar pa je možno le z sodim številom povezav, saj gremo z eno vedno v drugo množico in z drugo nazaj v prvo.
...
> Če vemo da je vsota stopenj vsakih 2 nepovezanih in neenakih vozlišč $u, v$ večja ali enaka številu vozlišč  potem velja da je graf hamiltonov natanko tedaj ko je $G + \{uv\}$ hamiltonov graf oz.
> $$\forall u,v \ni: u \neq v \land u \nsim v$$
> $$ \deg(u) + \deg(v)\geq |V| $$ 
> $$\Rightarrow $$ 
